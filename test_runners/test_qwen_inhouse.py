#!/usr/bin/env python3
"""
Qwen-inhouse model test runner for conversation analysis.
Uses in-house vLLM server (OpenAI-compatible /v1/chat/completions).
Results saved to results/qwen-inhouse/
"""
...
class QwenInhouseTestRunner:
    """Test runner for Qwen-inhouse (in-house vLLM) model"""
    ...
    def test_single_type(self, test_type: str, max_conversations: int = None) -> str:
        ...
        print(f"🧪 Running {test_type} test with Qwen-inhouse...")
    ...
    def test_all_types(self, max_conversations: int = None) -> Dict[str, str]:
        ...
        print("🚀 Running ALL conversation tests with Qwen-inhouse")
        print("=" * 60)
        ...
        for test_type in ASSESSMENT_PROMPTS.keys():
            print(f"\n{'='*50}")
            print(f"Testing {test_type.upper()} with Qwen-inhouse")
            print(f"{'='*50}")
    ...
    def run_interactive(self):
        ...
        print("🤖 Qwen-inhouse Model Test Runner")
        print("=" * 40)
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    print(f"\n{'='*60}")
                    print("🎯 ALL QWEN-INHOUSE TESTS COMPLETED!")
                    print(f"⏱️  Total time: {end_time - start_time}")
                    print("📁 Results saved in: results/qwen-inhouse/")
                    print(f"{'='*60}")
                    break
