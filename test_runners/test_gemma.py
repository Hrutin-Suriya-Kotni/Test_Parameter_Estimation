#!/usr/bin/env python3
"""
Gemma model test runner for conversation analysis
Tests all conversation types using google/gemma-2-9b via Hugging Face Inference provider featherless-ai
"""
...
class GemmaTestRunner:
    """Test runner for Gemma model"""

    def __init__(self):
        self.model_client = GemmaClient()
        self.test_runners = {}
    ...
    def test_single_type(self, test_type: str, max_conversations: int = None) -> str:
        ...
        print(f"🧪 Running {test_type} test with Gemma...")
    ...
    def test_all_types(self, max_conversations: int = None) -> Dict[str, str]:
        ...
        print("🚀 Running ALL conversation tests with Gemma")
        print("=" * 60)
        ...
        for test_type in ASSESSMENT_PROMPTS.keys():
            print(f"\n{'='*50}")
            print(f"Testing {test_type.upper()} with Gemma")
            print(f"{'='*50}")
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    print(f"\n{'='*60}")
                    print("🎯 ALL GEMMA TESTS COMPLETED!")
                    print(f"⏱️  Total time: {end_time - start_time}")
                    print("📁 Results saved in: results/gemma/")
                    print(f"{'='*60}")
                    break
