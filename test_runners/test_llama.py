#!/usr/bin/env python3
"""
Llama model test runner for conversation analysis
Tests all conversation types using Meta-Llama-3-8B-Instruct via Hugging Face router
"""
...
class LlamaTestRunner:
    """Test runner for Llama model"""

    def __init__(self):
        self.model_client = LlamaClient()
        self.test_runners = {}
    ...
    def test_single_type(self, test_type: str, max_conversations: int = None) -> str:
        ...
        print(f"🧪 Running {test_type} test with Llama...")
    ...
    def test_all_types(self, max_conversations: int = None) -> Dict[str, str]:
        ...
        print("🚀 Running ALL conversation tests with Llama")
        print("=" * 60)
        ...
        for test_type in ASSESSMENT_PROMPTS.keys():
            print(f"\n{'='*50}")
            print(f"Testing {test_type.upper()} with Llama")
            print(f"{'='*50}")
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    print(f"\n{'='*60}")
                    print(f"🎯 ALL LLAMA TESTS COMPLETED!")
                    print(f"⏱️  Total time: {end_time - start_time}")
                    print(f"📁 Results saved in: results/llama/")
                    print(f"{'='*60}")
                    break
