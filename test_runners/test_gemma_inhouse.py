#!/usr/bin/env python3
"""
Gemma-inhouse model test runner for conversation analysis.
Uses in-house vLLM server (OpenAI-compatible /v1/chat/completions).
Results saved to results/gemma-inhouse/
"""
...
class GemmaInhouseTestRunner:
    """Test runner for Gemma-inhouse (in-house vLLM) model"""

    def __init__(self):
        self.model_client = GemmaInhouseClient()
        self.test_runners = {}
    ...
    def test_single_type(self, test_type: str, max_conversations: int = None) -> str:
        ...
        print(f"🧪 Running {test_type} test with Gemma-inhouse...")
    ...
    def test_all_types(self, max_conversations: int = None) -> Dict[str, str]:
        ...
        print("🚀 Running ALL conversation tests with Gemma-inhouse")
        print("=" * 60)
        ...
        for test_type in ASSESSMENT_PROMPTS.keys():
            print(f"\n{'='*50}")
            print(f"Testing {test_type.upper()} with Gemma-inhouse")
            print(f"{'='*50}")
    ...
    def run_interactive(self):
        ...
        print("🤖 Gemma-inhouse Model Test Runner")
        print("=" * 40)
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    print(f"\n{'='*60}")
                    print("🎯 ALL GEMMA-INHOUSE TESTS COMPLETED!")
                    print(f"⏱️  Total time: {end_time - start_time}")
                    print("📁 Results saved in: results/gemma-inhouse/")
                    print(f"{'='*60}")
                    break
